...
这是一个把自动化测试和 AI 缺陷诊断结合起来的 Python portfolio 项目。首页是可以直接使用的 AI QA 诊断工作台：面试官可以粘贴 pytest、Playwright 或 API 失败日志，生成包含证据、分类、诊断假设和修复建议的中文报告。底层还用 FastAPI Demo Shop、pytest、Playwright 和 CI artifacts 展示完整自动化测试闭环。
...
1. 先打开 AI QA 诊断工作台：`http://127.0.0.1:8000`。
2. 粘贴或保留默认 pytest / Playwright / API 失败日志，点击“生成中文诊断报告”。
3. 打开 `/diagnosis-report`，讲 Failure Mode Matrix、证据、候选根因 / 诊断假设和下一步建议。
4. 指出 AI 诊断模式卡片：它能显示实时 AI 或 fallback 状态，但不暴露密钥和内部配置。
5. 如果面试官想验证被测系统，再进入 Demo Shop，登录 `alice / password123` 并创建订单。
6. 最后打开 `qa-reports` 或 `reports/examples/sample-ai-diagnosis.md`，说明 CI artifact 证据链。

这条线可以总结成：

```text
Workbench failure input -> 中文 AI 诊断报告 -> Failure Mode Matrix -> optional Demo Shop -> qa-reports
```
...
1. 历史诊断工作台：保存多次输入和报告，展示 failure mode 趋势。
2. Trace summarization：解析真实 Playwright trace、JUnit XML 或 Allure report，提供更完整证据。
3. Public deployment：部署一个只读 demo，让面试官不用本地运行也能体验工作台和 Failure Mode Matrix。
4. 更细的权限边界：区分 public demo、公司内网日志和第三方 provider 输入。
5. 自动修复建议草稿：只生成 patch idea 或测试补充建议，不直接替代人工 merge。
